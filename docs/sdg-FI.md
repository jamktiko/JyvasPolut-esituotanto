...
Sovelluksemme kannustaa ihmisiä liikkumaan luonnossa, jos luonto kärsii ihmisen toiminnan vuoksi, kaikki kärsivät, sekä sovelluksestamme tulee hyödytön, kun ei ole hyvinvoivaa luontoa missä liikkua.
...
Aiheemme edistää kestävää kehitystä sen pohjalta, että sovelluksemme ohjaa ihmistä mahdollisesti hakeutumaan luontoon ja sen myötä lisätä yksilön arvostusta ympäristö sekä luonnonsuojelua kohtaan ja ohjaa käyttäytymään kestävämmin arkisissa valinnoissaan.
...
Tässä tapauksessa sovellus on projekti luontoinen, ei ole välttämättä tarkoituskaan päätyä jakeluun, ei saavuta suurta käyttäjäkuntaa.
...
Sovellus kannustaa käyttäjää käyttämään julkista liikennettä luontopoluille mennessä.
...
Moderni koodikieli, ei käy käyttökelvottomaksi.
...
Käytetään universaaleja logoja= kaikille ymmärrettävissä, UI-layout helppokäyttöinen ja universaali.
...
Voi lisätä yhteiskunnallisella tasolla vastuullista arkikäyttäytymistä luontoa nähden.
...
Ei ajeta tiimiä burnikseen projektin luoman paineen alla.
